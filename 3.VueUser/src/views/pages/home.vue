	...
	<div>
		<div class="home_box">
			...
			<div class="recomList_view">
				<div class="recomList_title">餐厅信息推荐</div>
				<div class="recommend_list_one">
					<div class="recommend_item animation_box" v-for="(item,index) in cantingxinxiRecomList" :key="index" @click="detailClick('cantingxinxi',item.id)">
						<div class="recommend_img_box">
							<img class="recommend_img" v-if="isHttp(item.caipintupian)" :src="item.caipintupian.split(',')[0]" alt="">
							<img class="recommend_img" v-else :src="item.caipintupian?$config.url + item.caipintupian.split(',')[0]:''" alt="">
						</div>
						<div class="recommend_content">
							<div class="recommend_title">
								菜品名称：{{item.caipinmingcheng}}
							</div>
							<div class="recommend_title">
								{{item.caipinfenlei}}
							</div>
				...
				<div class="recommend_more_view" @click="moreClick('cantingxinxi')">
...
<script setup>
	...
	//餐厅信息推荐
	const cantingxinxiRecomList = ref([])
	const getcantingxinxiRecomList = () => {
		let autoSortUrl = 'cantingxinxi/autoSort'
		if(context?.$toolUtil.storageGet('frontToken')){
			autoSortUrl = "cantingxinxi/autoSort2"
		}
		context?.$http({
			url: autoSortUrl,
			method: 'get',
			...
			cantingxinxiRecomList.value = res.data.data.list
		})
	}
	//判断图片链接是否带http
	const isHttp = (str) => {
